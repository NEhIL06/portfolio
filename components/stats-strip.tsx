...
export function StatsStrip() {
    ...
    return (
        <div className="relative mx-auto max-w-5xl px-4 sm:px-6 py-16 border-y border-white/10 bg-gradient-to-r from-blue-500/5 via-black to-green-500/5 backdrop-blur-md overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 text-center divide-y md:divide-y-0 md:divide-x divide-white/5 relative z-10">
                {stats.map((stat, i) => (
                    <motion.div 
                        key={i} 
                        className="flex flex-col items-center justify-center py-6 md:py-0 min-w-[200px] min-h-[100px] md:min-h-[120px]"
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: i * 0.15 }}
                    >
                        <div className={`inline-flex items-center justify-center h-10 w-10 sm:h-12 sm:w-12 rounded-full ${colors[i]} border mb-3 sm:mb-4`}>
                        ...
                        <div className="text-3xl sm:text-4xl md:text-5xl font-bold bg-gradient-to-r from-white via-zinc-200 to-zinc-400 bg-clip-text text-transparent mb-2 sm:mb-3 font-mono tabular-nums">
